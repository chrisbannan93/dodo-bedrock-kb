# Approved EDM (Copy Extract)

**comm_type:** service
**campaign_type:** outage
**product:** internet
**audience_stage:** active
**date_sent:** 2026-02-01
**source:** sanitised internal HTML example

## Subject
Outage resolved for your service

## Preheader
We’re sorry if this outage has impacted you

## Body (rendered text; NO PII)
Hey <FIRSTNAME>

nbn® has resolved the outage affecting your service. We’re sorry if this outage has impacted you.

If your service is still not working, please reset your modem by turning it off and on at the power point.

You can also check our support documents at dodo.com/support or raise a support request via My Dodo.

Thanks
The Dodo team

## Notes (optional)
- CTA label: None
- Offer / key details: Outage resolved, troubleshooting steps
- Compliance notes: None
